...
class ddWeapon : Weapon
{
	...
	virtual ui void InventoryInfo(ddStats ddhud, bool debug)
	{
		...
		else
		{
			...
			hud.DrawString(hud.fa, "Altfire?: "..((fireMode) ? "Yes" : "No"), (32, 105), hud.DI_SCREEN_CENTER | hud.DI_TEXT_ALIGN_LEFT);
	...
	action void A_CheckLeftWeaponMag()
	{
		...
		{ 
			...
			ddp.player.SetPSprite(PSP_LEFTW0, lweap.FindState('Select'));
			ddp.player.SetPSprite(PSP_RIGHTW0, rweap.FindState('Select'));
		...
		if(ddp)
		{
			...
			else
			{
				if(ddp.rwx != rSwapTarget)
				{
					...
					ddp.player.SetPSprite(PSP_RIGHTW0, rWeap.GetUpState());
					ddp.player.SetPSprite(PSP_RIGHTWF0, null);
				}
			}
